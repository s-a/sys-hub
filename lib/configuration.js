...
Configuration.prototype.loadAndParseDataFromStorageFilename = function(cmd) {
	...
	switch(ext) {
		...
		case ".js":
			...
			break;
		default:
			var msg = colors.bgRed.white(cmd.argumentDatasource.filename + " not support as data storage");
			console.log(msg);
	}
	return jsonData;
};
...
Configuration.prototype.extendCommandsByType = function(setupTree) {
	...
	for (var key in setupTree) {
		...
		if (cmd.type){
			switch(cmd.type) {
				case "folder":
					this.extendCommandsByTypeFolder(cmd);
					break;
				case "file":
					this.extendCommandsByTypeFile(cmd);
					break;
			}
		} else {
