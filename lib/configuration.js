...
Configuration.prototype.parseConfigForArguments = function(cmd) {
	...
	if (cmd.argumentDatasource){
		...
		if (path.extname(cmd.argumentDatasource.filename).toLowerCase() === ".xml" || path.extname(cmd.argumentDatasource.filename).toLowerCase() === ".config"){
			jsonData = xml.parseString(rawData);
		} else {
			JSON.parse(cmd.argumentDatasource.filename);
		}
		...
		if (resultObj.length !== 0){
			...
			console.log(color("selector returned 0"), cmd);
			process.exit(1);
		}
...
Configuration.prototype.extendCommandsByType = function(setupTree) {
	...
	for (var key in setupTree) {
		...
		if (cmd.type){
			...
			switch(cmd.type) {
				case "folder":
					args = this.parseConfigForArguments(cmd);
