...
Configuration.prototype.pushCommandToHistory = function(path, cmd) {
	...
	console.log("AAAAAAAAAAAAAAAAAAAAA", path);
	var localCfg = JSON.parse(fs.readFileSync(this.local.filename).toString()) ;
	console.log("BBBBBBBBBBBBBBBBBBBB", localCfg.history);
	if (localCfg.history){
		for (var h = 0; h < localCfg.length; h++) {
			var history = localCfg.history[h];
			console.log("CCCCCCCCCCCCCCCCCCC", history);
		}
	}
};
